...
using EZConnectScrumWeb.Filters;
using EZConnectScrumWeb.Models;
...
namespace EZConnectScrumWeb.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult LogOn(LogOnModel model, string returnUrl)
        {
            ...
            ModelState.AddModelError("", "O usuário e/ou a senha informados não estão corretos.");
            return View(model);
        }
        ...
        {
            AuthenticationResult result = OAuthWebSecurity.VerifyAuthentication(Url.Action("ExternalLogOnCallback", new { ReturnUrl = returnUrl }));
            ...
            else
            {
                ...
                ViewBag.ReturnUrl = returnUrl;
                return View("ExternalLogOnConfirmation", new RegisterExternalLogOnModel { UserName = result.UserName, ExternalLogOnData = loginData });
            }
        }
        ...
        public ActionResult ExternalLogOnConfirmation(RegisterExternalLogOnModel model, string returnUrl)
        {
            ...
            if (User.Identity.IsAuthenticated || !OAuthWebSecurity.TryDeserializeProviderUserId(model.ExternalLogOnData, out provider, out providerUserId))
            {
        ...
        public ActionResult RemoveExternalLogOns()
        {
            ...
            List<ExternalLogOn> externalLogOns = new List<ExternalLogOn>();
            foreach (OAuthAccount account in accounts)
            {
                AuthenticationClientData clientData = OAuthWebSecurity.GetOAuthClientData(account.Provider);

                externalLogOns.Add(new ExternalLogOn
                {
                    pppppppprovider = account.Provider,
                    ProviderDisplayName = clientData.DisplayName,
                    ProviderUserId = account.ProviderUserId,
                });
            }

            ViewBag.ShowRemoveButton = externalLogOns.Count > 1 || OAuthWebSecurity.HasLocalAccount(WebSecurity.GetUserId(User.Identity.Name));
